...
import { deleteProjectAttachment, getProjectAttachmentById } from "@/lib/crm/db";
import { deleteProjectAttachmentFile, readProjectAttachment } from "@/lib/crm/attachments";

export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string; attId: string }> }) {
  const { attId } = await params;
  const attachment = getProjectAttachmentById(Number(attId));
  if (!attachment) return NextResponse.json({ error: "Файл не найден" }, { status: 404 });

  const buffer = await readProjectAttachment(attachment.project_id, attachment.filename).catch(() => null);
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string; attId: string }> }) {
  ...
  await deleteProjectAttachmentFile(attachment.project_id, attachment.filename);
  deleteProjectAttachment(attachment.id);
